class LevelSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    //Arrow
    const right = this.add.image(width - 60, height / 2, 'arrow').setScale(0.1);
    const left = this.add.image(60, height / 2, 'arrow').setScale(0.1).setAngle(180);

    //Select
    const select = this.add.image(width / 2, 600, 'select').setScale(1).setOrigin(0.5);
